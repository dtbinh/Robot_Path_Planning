function plot_scene_UAVnaval_onlygrid(TargetPos,SensorModel,curk)
hold on
...
%         plot_2Dcone(SensorModel.Static.pos{ij}(1),SensorModel.Static.pos{ij}(2),SensorModel.Static.FOV{ij}(1),SensorModel.Static.FOV{ij}(2),SensorModel.Static.FOV{ij}(3),'g','fill')
        plot_2Darc(SensorModel.Static.pos{ij}(1),SensorModel.Static.pos{ij}(2),SensorModel.Static.FOV{ij}(1),SensorModel.Static.FOV{ij}(2),SensorModel.Static.FOV{ij}(3),'g','fill')
    end
end
...
%         plot(Trajectories{sss}(:,1),Trajectories{sss}(:,2),SensorModel.Move.trajcol{sss})
        pos=[];
        for kk=1:1:curk
        pos=[pos;SensorModel.Move.Grid2Abs(SensorModel.Move.Gridpos{sss}(kk,1),SensorModel.Move.Gridpos{sss}(kk,2),SensorModel)];
        end
...
%             th=Nan;
%         end
        plot(pos(:,1),pos(:,2),SensorModel.Move.trajcol{sss})
        plot_ellipse(pos(end,1),pos(end,2),5,5,0,SensorModel.Move.trajcol{sss},'fill')
%         plot_triangle(pos(end,1),pos(end,2),6,1.5,th,SensorModel.Move.trajcol{sss},'no')
        plot_ellipse(pos(end,1),pos(end,2),SensorModel.Move.FOV{sss}(2),SensorModel.Move.FOV{sss}(2),0,SensorModel.Move.trajcol{sss},'fill')
    end
end
