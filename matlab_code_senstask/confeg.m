...
Z1=@(x)[norm([x(1)-s1(1),x(2)-s1(2)])];
Z2=@(x)[norm([x(1)-s2(1),x(2)-s2(2)])];
%Z3=@(x)[norm([x(1)-10,x(2)]);atan2(x(2),x(1)-10)];
% hjack=@(x,s)[(x(1)-s(1))/norm([x(1)-s(1),x(2)-s(2)]),(x(2)-s(2))/norm([x(1)-s(1),x(2)-s(2)]);(s(2)-x(2))/norm([x(1)-s(1),x(2)-s(2)])^2,(-s(1)+x(1))/norm([x(1)-s(1),x(2)-s(2)])^2];
...
hn=1;
nz=2;
H_x1=[hjack(mx1,s1);hjack(mx1,s2)];
H_x2=[hjack(mx2,s1);hjack(mx2,s2)];
...
Pz_x1=Pz_x1+blkdiag(R1,R2);
Pz_x2=Pz_x2+blkdiag(R1,R2);
...
cols={'vz1_x1','vz2_x1','vz1_x2','vz2_x2','log(|Px||Py|/|P|)','log(|Px|/|Px-PxyPyPxy|)','log(|Py|/|Py-PxyPxPxy|)','tr(Px)tr(Py)/tr(P)','tr(Px)/tr(Px-PxyPyPxy)','tr(Py)/tr(Py-PxyPxPxy)',...
...
for vz1_x1=0:1
    for vz1_x2=0:1
        for vz2_x1=0:1
            for vz2_x2=0:1
            if (vz1_x1+vz1_x2)*(vz2_x1+vz2_x2)~=1
                continue
            end
            %FIM
            FIM=inv(blkdiag(Px1,Px2))+[H_x1,zeros(2,2);zeros(2,2),H_x2]'*blkdiag(vz1_x1*inv(R1),vz2_x1*inv(R2),vz1_x2*inv(R1),vz2_x2*inv(R2))*[H_x1,zeros(2,2);zeros(2,2),H_x2];
            PP=inv(FIM)
            figure(kk)
            clf
                subplot(1,2,1)
                
                    plot_1sigellip([0,0],Px1,'r');
                    hold on
                    plot_1sigellip([0,0],PP(1:2,1:2),'b--');
                    title(strcat('Target 1 : ',num2str([vz1_x1,vz2_x1])))
                    hold off
                subplot(1,2,2)
                
                    plot_1sigellip([0,0],Px2,'r');
                    hold on
                    plot_1sigellip([0,0],PP(3:4,3:4),'b--');
                    title(strcat('Target 2 : ',num2str([vz1_x2,vz2_x2])))
                    hold off
             pause(0.2)
             ss=num2str([vz1_x1,vz1_x2,vz2_x1,vz2_x2]);
             ss(find(ss==' '))='_';
              saveas(gcf, strcat('COVellip_EKF_',ss), 'png')
              pause(0.3)
              
            % True infor
            if vz1_x1==0 && vz2_x1==0
            ...
            C{kk,1}=vz1_x1; C{kk,2}=vz2_x1; C{kk,3}=vz1_x2; C{kk,4}=vz2_x2; 
            C{kk,5}=log(det(Px1)*det(PPz_x1)/det(P1))+log(det(Px2)*det(PPz_x2)/det(P2)); 
            C{kk,6}=log(det(Px1)/det(InfoT_x1))+log(det(Px2)/det(InfoT_x2)); 
            C{kk,7}=log(det(PPz_x1)/det(InfoTzver_x1))+log(det(PPz_x2)/det(InfoTzver_x2)); 
            C{kk,8}=trace(Px1)*trace(PPz_x1)/trace(P1)+trace(Px2)*trace(PPz_x2)/trace(P2); 
            C{kk,9}=trace(Px1)/trace(InfoT_x1)+trace(Px2)/trace(InfoT_x2); 
            C{kk,10}=trace(PPz_x1)/trace(InfoTzver_x1)+trace(PPz_x2)/trace(InfoTzver_x2); 
            C{kk,11}=det(Px1)*det(PPz_x1)-det(P1)+det(Px2)*det(PPz_x2)-det(P2); 
            C{kk,12}=det(Px1)-det(InfoT_x1)+det(Px2)-det(InfoT_x2); 
            C{kk,13}=det(PPz_x1)-det(InfoTzver_x1)+det(PPz_x2)-det(InfoTzver_x2); 
            C{kk,14}=trace(Px1)*trace(PPz_x1)-trace(P1)+trace(Px2)*trace(PPz_x2)-trace(P2); 
            C{kk,15}=trace(Px1)-trace(InfoT_x1)+trace(Px2)-trace(InfoT_x2); 
            C{kk,16}=trace(PPz_x1)-trace(InfoTzver_x1)+trace(PPz_x2)-trace(InfoTzver_x2); 
            C{kk,17}=norm(Px1,'fro')^2*norm(PPz_x1,'fro')^2/norm(P1,'fro')^2+norm(Px2,'fro')^2*norm(PPz_x2,'fro')^2/norm(P2,'fro')^2; 
            C{kk,18}=norm(Px1,'fro')^2/norm(InfoT_x1,'fro')^2+norm(Px2,'fro')^2/norm(InfoT_x2,'fro')^2;  
            C{kk,19}=norm(PPz_x1,'fro')^2/norm(InfoTzver_x1,'fro')^2+norm(PPz_x2,'fro')^2/norm(InfoTzver_x2,'fro')^2; 
            C{kk,20}=norm(Px1,'fro')^2*norm(PPz_x1,'fro')^2-norm(P1,'fro')^2+norm(Px2,'fro')^2*norm(PPz_x2,'fro')^2-norm(P2,'fro')^2;
            
            C{kk,21}=(trace(Px1)+trace(Px2))*(trace(PPz_x1)+trace(PPz_x2))/(trace(P1)+trace(P2));
            C{kk,22}=(trace(Px1)*trace(PPz_x1)+trace(Px2)*trace(PPz_x2))/(trace(P1)+trace(P2));
            
            C{kk,23}=(trace(PPz_x1)+trace(PPz_x2))/(trace(InfoTzver_x1)+trace(InfoTzver_x2)); 
            
            C{kk,24}=(norm(Px1,'fro')^2+norm(Px2,'fro')^2)*(norm(PPz_x1,'fro')^2+norm(PPz_x2,'fro')^2)/(norm(P1,'fro')^2+norm(P2,'fro')^2) ;  
            C{kk,25}=(norm(Px1,'fro')^2*norm(PPz_x1,'fro')^2+norm(Px2,'fro')^2*norm(PPz_x2,'fro')^2)/(norm(P1,'fro')^2+norm(P2,'fro')^2); 
            C{kk,26}=(norm(Px1,'fro')^2+norm(Px2,'fro')^2)/(norm(InfoT_x1,'fro')^2+norm(InfoT_x2,'fro')^2) ;  
            C{kk,27}=(norm(PPz_x1,'fro')^2+norm(PPz_x2,'fro')^2)/(norm(InfoTzver_x1,'fro')^2+norm(InfoTzver_x2,'fro')^2); 
